...
Inside your NetHunter Rootless shell as your normal user:

```bash
sudo apt update
sudo apt install -y git curl wget unzip openjdk-21-jdk maven httpx-toolkit whatweb nuclei ffuf nikto sqlmap zaproxy golang-go
go install github.com/hahwul/dalfox/v2@latest
export PATH="$PATH:$(go env GOPATH)/bin"
echo 'export PATH="$PATH:$(go env GOPATH)/bin"' >> ~/.zshrc
```

Do not use `sudo su` for the rest of the setup. Keep running the backend as your normal NetHunter user. If you are in a `bash` shell, do not `source ~/.zshrc`; just keep the `export PATH=...` line for the current session.

Verify the required binaries:

```bash
which httpx-toolkit whatweb nuclei ffuf nikto dalfox sqlmap zaproxy
java -version
mvn -version
```
